...
### Agent
Prism 前端模块，负责监听、会话管理、负载均衡。详见 [[core/instance/overview|Agent 模块]]。

### AnyTLS
TLS 伪装方案，使用 padding 技术隐藏 TLS 流量特征。详见 [[ref/mihomo/protocols/anytls|AnyTLS]] 和 [[core/stealth/anytls/scheme|AnyTLS 实现]]。
...
### Buffer Pool
缓冲池，管理可复用内存块。详见 [[core/connect/overview|Channel 模块]]。
...
### Channel
Prism 连接管理模块，负责连接池、传输层、Happy Eyeballs。详见 [[core/connect/overview|Channel 模块]]。
...
### ECH
Encrypted Client Hello。TLS 扩展，加密握手信息。详见 [[ref/mihomo/protocols/ech|ECH]] 和 [[core/stealth/ech/config|ECH 实现]]。
...
### Front
Agent 子模块，负责监听和前端接收。详见 [[core/instance/front/listener|Listener]]。
...
### Happy Eyeballs
双栈连接算法，优先 IPv6 但快速 fallback IPv4。详见 [[ref/network/happy-eyeballs|Happy Eyeballs]] 和 [[core/connect/dial/racer|Racer 实现]]。
...
### Listener
监听器，接收新连接。详见 [[core/instance/front/listener|Listener]]。
...
### Restls
TLS 伪装方案，模拟 TLS 响应时序。详见 [[ref/mihomo/transport/restls|Restls]] 和 [[core/stealth/restls/scheme|Restls 实现]]。
...
### ShadowTLS
TLS 伪装方案，使用真实 TLS 握手包装代理协议。详见 [[ref/mihomo/transport/shadowtls|ShadowTLS]] 和 [[core/stealth/shadowtls/overview|ShadowTLS 实现]]。
...
### TrustTunnel
TLS 伪装方案，使用自定义 TLS 扩展。详见 [[ref/mihomo/protocols/trusttunnel|TrustTunnel]] 和 [[core/stealth/trusttunnel/overview|TrustTunnel 实现]]。
