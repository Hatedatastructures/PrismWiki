...
`reliable` 类继承自 [[core/transport/transmission|transmission]]，是分层流式架构中的具体传输层实现。所有异步操作返回 `net::awaitable`，简化异步操作调用。
...
- 被 [[core/transport/encrypted|encrypted]] TLS 加密传输层使用
