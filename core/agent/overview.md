...
| 组件 | 文件 | 职责 |
|------|------|------|
| [[core/agent/front/listener\|listener]] | `front/listener.hpp` | 入站连接接受 |
| [[core/agent/front/balancer\|balancer]] | `front/balancer.hpp` | Worker 选择与负载均衡 |
| [[core/agent/worker/worker\|worker]] | `worker/worker.hpp` | 线程事件循环 |
| [[core/agent/worker/launch\|launch]] | `worker/launch.hpp` | 会话创建与分发 |
| [[core/agent/worker/tls\|tls]] | `worker/tls.hpp` | SSL 上下文构建 |
| [[core/agent/session/session\|session]] | `session/session.hpp` | 连接生命周期 |
