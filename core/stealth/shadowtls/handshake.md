...
- **传输阶段写入方向发 plain payload（不加密）**：仅依赖外层 TLS 保护，排错时需注意
- **relay 协程 use-after-close 竞态**：500ms 超时后 relay 协程可能仍在访问已关闭的 backend socket 或写入 client socket（handshake.cpp 行 557-586）
- **累积 HMAC 无重置**：HMAC_Update 累积所有 payload，CPU 开销线性增长，长连接传输 GB 级数据后性能显著退化
- **TLS frame 长度无上限检查**：`record_length = (raw[3] << 8) | raw[4]` 最大 65535 字节

日志关键词：`socket may be corrupted`、`HMAC mismatch`、`read TLS header/payload failed`

详见 [[dev/debugging/deep-dive/shadowtls-limitations|ShadowTLS 深层限制与故障分析]]
