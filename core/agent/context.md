...
layer: core
source: "I:/code/Prism/include/prism/agent/context.hpp"
created: 2026-05-17
...
- `channel::transport` — session_context.inbound/outbound 持有
...
worker_context 在会话中使用:
    ...
    ├── channel::allocate() → ctx_.worker.memory_pool
```
...
launch::dispatch(ioc, server_ctx, worker_ctx, metrics, socket)  // launch.cpp
    │
    ├── 创建传输层
    │       auto transmission = channel::transport::make_reliable(socket, ...);
    │
    ├── 构造 session_params
