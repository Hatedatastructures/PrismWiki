...
**调用链**:
- **调用**（向下）:
  - [[crypto/aead#函数: encrypt]] — 加密握手数据
  - [[protocol/common/address#函数: parse]] — 解析目标地址
- **被调用**（向上）:
  - [[agent/session/session#函数: start]] — session 启动时调用
...
- **被调用**（向上）:
  - [[stealth/scheme#函数: try_scheme]] — 伪装方案尝试
```
...
| 依赖方向 | 模块 | 说明 |
|----------|------|------|
| 被依赖 | [[multiplex/yamux/craft]] | 帧构造使用配置参数 |
| 被依赖 | [[multiplex/core]] | 核心层加载配置 |
...
| 依赖方向 | 模块 | 说明 |
|----------|------|------|
| 被依赖 | [[stealth/reality/auth]] | Reality 认证数据编码 |
| 被依赖 | [[protocol/trojan/relay]] | Trojan 凭据编码 |
...
**调用链**:
- **调用**: 无外部依赖
- **被调用**: [[stealth/reality/auth#函数: sign]]
...
**调用链**:
- **调用**: 无外部依赖
- **被调用**: [[protocol/trojan/relay#函数: verify_credential]]
```
...
| 依赖方向 | 模块 | 说明 |
|----------|------|------|
| 依赖 | [[transformer/json]] | glaze JSON 反序列化 |
...
相关实现文件：[[stealth/shadowtls/auth|auth]]、[[stealth/shadowtls/handshake|handshake]]
...
- [[stealth/shadowtls/auth]] — 认证实现
- [[stealth/shadowtls/handshake]] — 握手实现
```
...
| 函数/类 | 使用方式 | 链接 |
|---------|----------|------|
| verify_client_hello | 验证 SessionID | [[stealth/shadowtls/auth|auth]] |
| compute_hmac | 计算 HMAC 标签 | [[stealth/shadowtls/auth|auth]] |
...
- [[dev/modules]] — 模块结构
- [[channel/transport/reliable]] — TCP 传输层
```
