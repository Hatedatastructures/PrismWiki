...
`unreliable` 类继承自 [[core/transport/transmission|transmission]]，实现基于 UDP 的数据报传输。由于 UDP 是无连接的，该类内部维护一个远程端点（remote endpoint），所有发送操作都指向该端点，接收操作则验证来源是否匹配。
...
- 与 [[core/transport/reliable|reliable]] TCP 可靠传输对应
