...
| 模块 | 用途 |
|------|------|
| [[core/agent/config|config]] | 提供证书/私钥路径配置 |

### 被引用

| 模块 | 使用方式 |
|------|----------|
| [[core/agent/worker/worker|worker]] | 构造时调用 `tls::make(cfg)` 获取 `ssl_ctx_` |
| [[core/agent/worker/launch|launch]] | session 启动时检查 `ssl_ctx_` 是否存在以决定 TLS/明文路径 |
