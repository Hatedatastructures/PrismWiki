...
related:
  ...
  - core/channel/adapter/connector
  - core/channel/eyeball/racer
---

# channel — 通道阶段概述

> 源码目录: `I:/code/Prism/include/prism/channel/`
> 模块: [[channel|channel]]
> 定位: 请求处理流程的第五阶段
...
| 职责 | 说明 | 关键组件 |
|------|------|----------|
| 传输层抽象 | 统一 TCP/UDP/TLS 的读写接口 | [[core/channel/transport/transmission|transmission]] |
| TCP 可靠传输 | 封装 TCP socket，支持连接池复用 | [[core/channel/transport/reliable|reliable]] |
| TLS 加密传输 | 封装 TLS 流，提供加密读写 | [[core/channel/transport/encrypted|encrypted]] |
| UDP 数据报传输 | 封装 UDP socket，模拟连接式操作 | [[core/channel/transport/unreliable|unreliable]] |
| 预读回滚传输 | 捕获读取数据，支持 rewind 重试 | [[core/channel/transport/snapshot|snapshot]] |
| 连接池管理 | LIFO 栈式缓存，健康检测，后台清理 | [[core/channel/connection/pool|pool]] |
| Happy Eyeballs | 多地址并发竞速连接 | [[core/channel/eyeball/racer|racer]] |
| Socket 适配器 | 适配 Boost.Asio 流概念 | [[core/channel/adapter/connector|connector]] |
| 健康检测 | Socket 状态检测，防止复用僵尸连接 | [[core/channel/health|health]] |
...
| [[core/channel/health|health]] | `healthy_fast()` | 连接池复用前健康检测 |
...
[[core/channel/transport/transmission|transmission]] 是所有传输层的抽象基类，采用纯协程设计：
...
[[core/channel/connection/pool|connection_pool]] 采用 LIFO 栈式缓存：
...
[[core/channel/eyeball/racer|address_racer]] 实现 RFC 8305 并发连接：
...
- [[core/channel/health|health]] — Socket 健康检测
- [[core/channel/transport/transmission|transmission]] — 传输层抽象接口
- [[core/channel/transport/reliable|reliable]] — TCP 可靠传输
- [[core/channel/transport/encrypted|encrypted]] — TLS 加密传输
- [[core/channel/transport/unreliable|unreliable]] — UDP 数据报传输
- [[core/channel/transport/snapshot|snapshot]] — 预读回滚传输
- [[core/channel/connection/pool|pool]] — TCP 连接池
- [[core/channel/eyeball/racer|racer]] — Happy Eyeballs 竞速器
- [[core/channel/adapter/connector|connector]] — Boost.Asio 适配器