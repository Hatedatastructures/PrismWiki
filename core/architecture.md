...
详见 [[core/instance/front/listener|监听器]]、[[core/instance/front/balancer|负载均衡器]]
...
详见 [[core/instance/worker/worker|Worker]]、[[core/instance/worker/launch|会话启动]]

### Session Layer（会话层）

- **session**: 会话生命周期管理，持有 transport

详见 [[core/instance/session/session|会话生命周期]]
...
详见 [[core/recognition/recognition|协议识别入口]]、[[core/recognition/layered_pipeline|分层检测管道]]
...
详见 [[core/resolve/router|DNS 路由器]]、[[core/resolve/dns/dns|DNS 解析入口]]
...
详见 [[core/multiplex/overview|多路复用总览]]
...
详见 [[core/connect/overview|通道层总览]]、[[core/connect/pool/pool|连接池]]
...
详见 [[core/stealth/overview|伪装层总览]]
