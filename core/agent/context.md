...
| 调用者 | 用途 |
|--------|------|
| [[core/agent/worker/worker|worker.hpp]] | 持有 server + worker 上下文 |
| [[core/agent/worker/launch|launch.hpp]] | 构造 session_opts 传递上下文 |
| [[core/agent/session/session|session.hpp]] | 持有 session 上下文 |
...
- [[core/agent/config|配置类型定义]] — server.cfg 持有的类型
- [[core/agent/worker/worker|Worker]] — 持有 server + worker 上下文
- [[core/agent/session/session|会话]] — 持有 session 上下文
