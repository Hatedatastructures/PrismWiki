...
PMR（Polymorphic Memory Resource，多态内存资源）是 C++17 引入的标准库内存分配器框架，为高性能网络应用提供了灵活且高效的内存管理能力。Prism 代理引擎采用 C++23 纯协程架构，结合 PMR 实现了热路径零堆分配的目标，彻底消除了数据转发路径上的内存分配瓶颈。

## 概述

### 什么是 PMR

传统 C++ 内存管理依赖全局 `new/delete` 操作符，每次分配都需要与操作系统内核交互或走通用分配器（如 glibc malloc、jemalloc、mimalloc）。这些分配器虽然经过高度优化，但对于高频小对象分配场景仍存在显著开销：

- **分配延迟**：单次 malloc/free 调用需要 50-200 纳秒
- **锁竞争**：多线程环境下分配器内部锁成为瓶颈
- **内存碎片**：长期运行后堆内存碎片化，降低内存利用率

PMR 通过引入 `std::pmr::memory_resource` 抽象基类，允许在运行时切换底层内存分配策略。容器使用 `std::pmr::polymorphic_allocator` 分配器，可以绑定到任意 `memory_resource` 实现：
...
│  │  │   ┌─────────────────┬──────────────────────────┐    ││ │
│  │  │   │ new_delete      │ synchronized_pool        │    ││ │
│  │  │   │ monotonic_buffer│ unsynchronized_pool      │    ││ │
│  │  │   │ null_resource   │ custom implementation    │    ││ │
│  │  │   └─────────────────┴──────────────────────────┘    ││ │
│  │  └─────────────────────────────────────────────────────┘│ │
│  └─────────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────┘
...
1. **类型安全的多态性**：通过虚函数调用实现运行时策略切换，无需模板参数传递
2. **池化分配**：预分配固定大小块，避免频繁系统调用
3. **线程隔离**：每个线程独立的内存池，消除分配器锁竞争
4. **批量回收**：`monotonic_buffer_resource` 支持一次性释放所有分配
5. **可定制策略**：自定义 `memory_resource` 实现特定场景优化

### 为什么代理引擎需要 PMR

代理引擎是典型的高并发 I/O 密集型应用，内存管理面临独特挑战：

#### 高频小对象分配

每个代理连接在其生命周期内产生大量小对象：协议头解析缓冲区、DNS 查询结果、TLS 会话状态、连接元数据等。一个处理 10 万并发连接的引擎，每秒可能产生数百万次内存分配。以 HTTP 代理为例，单次请求可能触发以下分配序列：

```
连接建立 → 接收 ClientHello (512B-4KB) → 解析 HTTP Header (256B-2KB)
           ↓
建立上游连接 → DNS 查询缓存 (64B-256B) → TLS 握手状态 (1KB-4KB)
           ↓
双向转发 → 接收缓冲区 (4KB-16KB) × N → 发送缓冲区 (4KB-16KB) × N
           ↓
连接关闭 → 释放所有对象
```

假设每秒处理 5000 个请求，每个请求平均触发 50 次分配，则每秒产生 250000 次 malloc/free 调用。传统分配器的开销累积成为显著瓶颈。

#### 多线程锁竞争

通用分配器使用全局锁或分桶锁管理堆内存。在多线程代理引擎中，多个工作线程同时分配/释放内存导致严重锁竞争：

```
Thread A → malloc(256) → lock arena_0 → allocate → unlock
Thread B → malloc(256) → lock arena_0 → wait...      → allocate → unlock
Thread C → free(ptr)    → lock arena_0 → wait...      → deallocate → unlock
Thread D → malloc(512) → lock arena_1 → allocate → unlock
```

PMR 的 `unsynchronized_pool_resource` 为每个线程提供独立内存池，分配和释放完全无锁：

```
Thread A → pool_A.allocate(256) → 无锁，直接返回
Thread B → pool_B.allocate(256) → 无锁，直接返回
Thread C → pool_C.deallocate(ptr) → 无锁，直接归还
Thread D → pool_D.allocate(512) → 无锁，直接返回
```

#### 内存碎片控制

长期运行的代理引擎经历连接频繁创建和销毁，导致堆内存碎片化。碎片化带来两个问题：

1. **内存浪费**：碎片间隙无法利用，实际占用远超工作集
2. **分配延迟增加**：碎片化后分配器需要更复杂的搜索逻辑

PMR 的 Pool Resource 使用固定大小的块分配，天然抗碎片。Monotonic Buffer Resource 则完全不单独释放内存，通过整体重置回收，从根本上避免碎片。

### Prism 的 PMR 设计目标

Prism 代理引擎采用以下内存策略：

1. **热路径零分配**：数据转发路径上不调用任何 `new/delete/malloc/free`
2. **线程封闭**：每个 worker 线程独立内存池，消除分配器锁竞争
3. **大小分类**：池化常用大小（64B、256B、1KB、4KB），大对象直通系统堆
4. **帧生命周期**：短生命周期对象使用 frame arena，批量回收
5. **全局池兜底**：跨线程对象使用全局同步池
...
C++17 标准库提供四种 `memory_resource` 实现：
...
| thread_local_pool × 4 | 256KB × 4 | ~1.12MB | ~91% |
...
- [[dev/cpp23-coroutines]] — 协程架构，协程帧也从 PMR 池分配
- [[dev/agent]] — Agent 模块，使用 PMR 的会话管理
- [[dev/tcp]] — 传输层协议，每个 TCP 连接的缓冲区使用 PMR
- [[docs/prism/memory]] — Prism 内存系统详细文档
- [[docs/prism/performance-report]] — 性能基准测试报告